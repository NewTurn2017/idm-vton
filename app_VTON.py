...
parser.add_argument("--share", type=str, default=False,
                    help="Set to True to share the app publicly.")
parser.add_argument("--lowvram", action="store_true",
                    help="Enable CPU offload for model operations.")
parser.add_argument("--load_mode", default=None, type=str, choices=[
                    "4bit", "8bit"], help="Quantization mode for optimization memory consumption")
parser.add_argument("--fixed_vae", action="store_true",
                    default=True,  help="Use fixed vae for FP16.")
...
def start_tryon(dict, garm_img, garment_des, category, is_checked, is_checked_crop, denoise_steps, is_randomize_seed, seed, number_of_images):
    ...
    if pipe == None:
        ...
        if fixed_vae:
            vae = AutoencoderKL.from_pretrained(
                vae_model_id, torch_dtype=dtype)
        else:
    ...
    if is_checked:
        ...
        mask, mask_gray = get_mask_location(
            'hd', category, model_parse, keypoints)
        ...
        with torch.cuda.amp.autocast(dtype=dtype):
            with torch.no_grad():
                ...
                with torch.inference_mode():
                    ...
                    pose_img = tensor_transfrom(
                        pose_img).unsqueeze(0).to(device, dtype)
                    garm_tensor = tensor_transfrom(
                        garm_img).unsqueeze(0).to(device, dtype)
                    results = []
                    current_seed = seed
                    for i in range(number_of_images):
                        if is_randomize_seed:
                            current_seed = torch.randint(
                                0, 2**32, size=(1,)).item()
                        generator = torch.Generator(device).manual_seed(
                            current_seed) if seed != -1 else None
                        ...
                        if is_checked_crop:
                            ...
                            human_img_orig.paste(
                                out_img, (int(left), int(top)))
                            img_path = save_output_image(
                                human_img_orig, base_path="outputs", base_filename='img', seed=current_seed)
...
garm_list_path = [os.path.join(example_path, "cloth", garm)
                  for garm in garm_list]

human_list = os.listdir(os.path.join(example_path, "human"))
human_list_path = [os.path.join(example_path, "human", human)
...
with image_blocks as demo:
    ...
    with gr.Row():
        with gr.Column():
            imgs = gr.ImageEditor(sources='upload', type="pil",
                                  label='인물 이미지. 펜으로 마스크하거나 자동 마스킹 사용', interactive=True)
            with gr.Row():
                category = gr.Radio(choices=[
                                    "upper_body", "lower_body", "dresses"], label="의류 카테고리 선택", value="upper_body")
                is_checked = gr.Checkbox(
                    label="예", info="자동 생성 마스크 사용 (5초 소요)", value=True)
            with gr.Row():
                is_checked_crop = gr.Checkbox(
                    label="예", info="자동 자르기 및 크기 조정 사용", value=True)
        ...
        with gr.Column():
            ...
            with gr.Row(elem_id="prompt-container"):
                with gr.Row():
                    prompt = gr.Textbox(
                        placeholder="의류 설명 예) 반팔 라운드넥 티셔츠", show_label=False, elem_id="prompt")
            example = gr.Examples(
                inputs=garm_img,
                examples_per_page=8,
        ...
        with gr.Column():
            with gr.Row():
                masked_img = gr.Image(
                    label="마스크 이미지 출력", elem_id="masked-img", show_share_button=False)
            with gr.Row():
                btn_open_outputs = gr.Button("출력 폴더 열기")
                btn_open_outputs.click(fn=open_folder)
        with gr.Column():
            with gr.Row():
                image_gallery = gr.Gallery(
                    label="생성된 이미지", show_label=True)
            with gr.Row():
                try_button = gr.Button(value="가상 착용 시작")
                denoise_steps = gr.Number(
                    label="디노이징 단계", minimum=20, maximum=120, value=30, step=1)
                seed = gr.Number(label="시드", minimum=-1,
                                 maximum=2147483647, step=1, value=1)
                is_randomize_seed = gr.Checkbox(
                    label="생성된 각 이미지에 대해 시드 무작위화", value=True)
                number_of_images = gr.Number(
                    label="생성할 이미지 수 (입력한 시드에서 시작하여 1씩 증가)", minimum=1, maximum=9999, value=1, step=1)

    try_button.click(fn=start_tryon, inputs=[imgs, garm_img, prompt, category, is_checked, is_checked_crop, denoise_steps,
                     is_randomize_seed, seed, number_of_images], outputs=[image_gallery, masked_img], api_name='tryon')
